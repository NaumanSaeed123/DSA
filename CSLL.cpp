...
public:
    ...
    {
        if (head == nullptr) // If the list is empty, return false
        {
            return false;
        }

        bool deletedAny = false; // Flag to track if any nodes were deleted
        Node<T>* current = head;
        Node<T>* prev = nullptr;

        // Handle deletion if the head node is even
        while (current->info % 2 == 0)
        {
            deletedAny = true;
            if (current->next == head)
            {
                // If there's only one node and it's even
                delete head;
                head = nullptr;
                return true;
            }
        ...
        // Now process the rest of the list for even numbers
        prev = current;
        current = current->next;
...
int main()
{
    ...
    /*Node<int>* mergedHead = ld.mergeLinkedLists(ld.search(1), ld2.search(5));

    
    Node<int>* head = mergedHead;
    if (head != nullptr)
    {
